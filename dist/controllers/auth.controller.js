...
exports.getMyProfile = exports.logout = exports.refreshToken = exports.register = exports.googleLogin = exports.login = void 0;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
const crypto_1 = __importDefault(require("crypto"));
const healpers_1 = __importDefault(require("../lib/healpers"));
const db_1 = __importDefault(require("../lib/db"));
const user_schema_1 = require("../zodSchemas/user.schema");
...
function generateTokens(payload) {
    ...
    const accessToken = jsonwebtoken_1.default.sign(tokenPayload, ACCESS_JWT_SECRET, {
        expiresIn: ACCESS_TOKEN_EXPIRY,
    });
    const refreshToken = jsonwebtoken_1.default.sign({ ...tokenPayload, jti: crypto_1.default.randomUUID() }, REFRESH_JWT_SECRET, {
...
const loginSchema = user_schema_1.userSchema.pick({ email: true, password: true });
exports.login = (0, healpers_1.default)(async (req, res) => {
    ...
    try {
        ...
        const isValidPassword = isValidUser ? await bcryptjs_1.default.compare(password, user.passwordHash) : false;
        // Always perform bcrypt comparison even if user doesn't exist (timing attack prevention)
        if (!isValidUser) {
            await bcryptjs_1.default.compare(password, '$2a$10$fake.hash.to.prevent.timing.attacks.dummy.hash');
        }
...
exports.googleLogin = (0, healpers_1.default)(async (req, res, next) => {
    try {
        // Validate input
        ...
        if (!emailRegex.test(normalizedEmail)) {
            return next(new utils_1.ErrorHandler("Invalid email format", 400));
        }
        ...
        const [user, client] = await Promise.all([
            ...
            db_1.default.client.findUnique({
                where: { email: normalizedEmail },
                select: {
                    id: true,
                    name: true,
        ...
        if (!account || !account.isActive) {
            return next(new utils_1.ErrorHandler("Account not found or inactive", 404));
        }
        // Update last login
        const updatePromise = user
            ? db_1.default.user.update({ where: { id: account.id }, data: { lastLoginAt: new Date() } })
            : db_1.default.client.update({ where: { id: account.id }, data: { lastLoginAt: new Date() } });
        await updatePromise;
        // Generate tokens
        const tokenPayload = {
            id: account.id,
        ...
        return next(new utils_1.ErrorHandler("Login failed", 500));
    }
});
exports.register = (0, healpers_1.default)(async (req, res, next) => {
    try {
        // Validate input
        const validatedData = user_schema_1.userSchema.parse(req.body);
        ...
        if (existingUser) {
            return next(new utils_1.ErrorHandler("User with this email already exists", 409));
        }
...
exports.refreshToken = (0, healpers_1.default)(async (req, res, next) => {
    ...
    if (!refreshToken) {
        return res.status(401).json(createErrorResponse("Refresh token missing", 401));
    }
    try {
        // Verify refresh token with refresh secret
        const decoded = jsonwebtoken_1.default.verify(refreshToken, REFRESH_JWT_SECRET);
        if (!decoded?.id || !decoded?.role) {
            return res.status(403).json(createErrorResponse("Invalid refresh token payload", 403));
        }
        ...
        if (!account || !account.isActive) {
            return res.status(404).json(createErrorResponse("Account not found or inactive", 404));
        }
        ...
        if (err.name === 'TokenExpiredError') {
            return res.status(403).json(createErrorResponse("Refresh token has expired", 403));
        }
        else if (err.name === 'JsonWebTokenError') {
            return res.status(403).json(createErrorResponse("Invalid refresh token", 403));
        }
        else {
            return res.status(403).json(createErrorResponse("Token verification failed", 403));
        }
    }
});
exports.logout = (0, healpers_1.default)(async (req, res) => {
...
exports.getMyProfile = (0, healpers_1.default)(async (req, res, next) => {
    // This assumes you have auth middleware that sets req.user
    const userId = req.user?.id || req.user?.id;
    if (!userId) {
        return next(new utils_1.ErrorHandler("Authentication required", 401));
    }
    try {
        const user = await db_1.default.user.findUnique({
            where: { id: userId },
        ...
        return next(new utils_1.ErrorHandler("Failed to fetch profile", 500));
    }
});
