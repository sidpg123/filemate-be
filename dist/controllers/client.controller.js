...
exports.getDocuments = exports.uploadDocMetaData = exports.getFeeStatistics = exports.deleteFeeRecord = exports.updateFeeRecord = exports.addFeeRecord = exports.getFeeRecords = exports.deleteClient = exports.updateClient = exports.getClientById = exports.addClient = exports.getClients = void 0;
...
exports.getClients = (0, healpers_1.default)(async (req, res, next) => {
    ...
    const feesStatusFilter = req.query.feeStatus; // Pending, Paid
    const sortBy = req.query.sortBy; // pendingPayment, name, createdAt
    const sortOrder = req.query.sortOrder || "desc";
    let cursor;
    if (rawCursorCreatedAt && rawCursorId) {
        cursor = {
            createdAt: new Date(rawCursorCreatedAt),
            id: rawCursorId
        };
    }
...
//     });
// });
exports.addClient = (0, healpers_1.default)(async (req, res, next) => {
    //("Adding new client...");
    ...
    if (existingClient || existingClient1) {
        return next(new utils_1.ErrorHandler("Client with this email already exists", 400));
    }
    console.log("userPlan", userPlan);
    if (userPlan?.plan.name === 'ff') {
        const clientCount = await db_1.default.client.count({
            where: {
                caId: userId,
            }
        });
        ...
        if (clientCount > 1) {
            return next(new utils_1.ErrorHandler("Free plan allows only 10 clients. Please upgrade your plan.", 400));
        }
    }
    const newClient = await db_1.default.client.create({
        data: {
            caId: userId,
            name,
            email,
...
exports.getClientById = (0, healpers_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const userId = req.user?.id;
    if (!userId) {
        return next(new utils_1.ErrorHandler("Unauthorized: User ID not found", 401));
    ...
    if (!client) {
        return next(new utils_1.ErrorHandler("Client not found", 404));
    }
    const today = new Date();
    const allFees = await db_1.default.pendingFees.findMany({
        where: {
            clientId: id,
            client: { caId: userId }
        },
...
exports.updateClient = (0, healpers_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const userId = req.user?.id; // Assuming user ID is stored in req.user by authentication middleware
    if (!userId) {
        return next(new utils_1.ErrorHandler("Unauthorized: User ID not found", 401));
...
exports.deleteClient = (0, healpers_1.default)(async (req, res, next) => {
    const { id } = req.params;
    const userId = req.user?.id; // Assuming user ID is stored in req.user by authentication middleware
    if (!userId) {
        return next(new utils_1.ErrorHandler("Unauthorized: User ID not found", 401));
    ...
    const deletedClient = await db_1.default.client.delete({
        where: {
            id: id,
            caId: userId,
        },
...
exports.getFeeRecords = (0, healpers_1.default)(async (req, res, next) => {
    ...
    const userId = req.user?.id;
    const { cursorId, cursorCreatedAt, limit = '10', search, status, page = '1', feeCategoryId } = req.query;
    if (!userId) {
        return next(new utils_1.ErrorHandler("Unauthorized: User ID not found", 401));
    }
    if (!id) {
    ...
    if (!clientExists) {
        return next(new utils_1.ErrorHandler("Client not found or does not belong to you", 404));
    }
    let whereClause = {
        clientId: id,
        client: { caId: userId }
...
exports.addFeeRecord = (0, healpers_1.default)(async (req, res, next) => {
    ...
    if (!amount || !dueDate) {
        return next(new utils_1.ErrorHandler("Amount and due date are required", 400));
    }
    // Validate amount
    const parsedAmount = parseFloat(amount);
    if (isNaN(parsedAmount) || parsedAmount <= 0) {
        return next(new utils_1.ErrorHandler("Amount must be a positive number", 400));
    }
    // Validate status
    const validStatuses = ['Pending', 'Paid'];
    if (!validStatuses.includes(status)) {
        return next(new utils_1.ErrorHandler("Invalid status. Must be Pending, Paid, or Overdue", 400));
    }
    // Validate dates
    const parsedDueDate = new Date(dueDate);
    if (isNaN(parsedDueDate.getTime())) {
        return next(new utils_1.ErrorHandler("Invalid due date format", 400));
    }
    let parsedPaymentDate = null;
    if (paymentDate) {
        parsedPaymentDate = new Date(paymentDate);
        if (isNaN(parsedPaymentDate.getTime())) {
            return next(new utils_1.ErrorHandler("Invalid payment date format", 400));
        }
    }
    ...
    try {
        ...
        const isClientExists = await db_1.default.client.findUnique({
            where: {
                id: id,
                caId: userId,
            },
        ...
        if (!isClientExists) {
            return next(new utils_1.ErrorHandler("Client not found or unauthorized", 404));
        }
        const response = await db_1.default.pendingFees.create({
            data: {
                amount: parsedAmount,
                note: note?.trim() || null,
        ...
        return next(new utils_1.ErrorHandler("Failed to add fee record", 500));
    }
});
exports.updateFeeRecord = (0, healpers_1.default)(async (req, res, next) => {
    ...
    if (!feeId) {
        return next(new utils_1.ErrorHandler("Fee record ID is required", 400));
    }
    if (!userId) {
        return next(new utils_1.ErrorHandler("Unauthorized: User ID not found", 401));
    }
    ...
    const validStatuses = ['Pending', 'Paid'];
    if (!validStatuses.includes(status)) {
        return next(new utils_1.ErrorHandler("Invalid status. Must be Pending or Paid", 400));
    }
    // Validate dates
    const parsedDueDate = new Date(dueDate);
    if (isNaN(parsedDueDate.getTime())) {
    ...
    try {
        ...
        const isFeeRecordExists = await db_1.default.pendingFees.findFirst({
            where: {
                id: feeId,
                clientId: id,
                client: {
        ...
        if (!isFeeRecordExists) {
            return next(new utils_1.ErrorHandler("Fee record not found or unauthorized", 404));
        }
        ...
        const response = await db_1.default.pendingFees.update({
            where: {
                id: feeId,
            },
            data: {
        ...
        return next(new utils_1.ErrorHandler("Failed to update fee record", 500));
    }
});
exports.deleteFeeRecord = (0, healpers_1.default)(async (req, res, next) => {
    ...
    try {
        ...
        const delData = await db_1.default.pendingFees.delete({
            where: {
                id: feeId,
            },
        });
        ...
        return next(new utils_1.ErrorHandler("Failed to delete fee record", 500));
    }
});
...
exports.uploadDocMetaData = (0, healpers_1.default)(async (req, res, next) => {
    const userId = req.user?.id;
    ...
    if (!clientId) {
        return next(new utils_1.ErrorHandler("ClientId not found", 400));
    }
